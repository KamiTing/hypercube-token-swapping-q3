...
CSV_PATH = "output/q4_random_benchmark.csv"
OUT_DIR = "output"
...
def main():
    ...
    bar_w = 0.28
    x = steps
    x_astar = [v - bar_w for v in x]
    x_beam = x
    x_batcher = [v + bar_w for v in x]

    plt.bar(x_astar, astar_counts.values, width=bar_w, label="A* steps")
    ...
    data = [df["astar_sec"], df["beam_sec"], df["batcher_sec"]]
    plt.boxplot(data, labels=["A*", "Beam", "Batcher"], showfliers=False)
    ...
    if len(astar_gap) > 0:
        plt.hist(astar_gap, bins=30, alpha=0.5, label="A* - Batcher")
    if len(beam_gap) > 0:
